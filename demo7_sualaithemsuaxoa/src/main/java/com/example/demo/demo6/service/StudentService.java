...
//                        .clazz(clazzMapper.toDto(student.getClazz()))
    ...
    public String updateStudent(int id, StudentDTO studentDTO) {
        ...
        // Nếu sinh viên không tồn tại, trả về thông báo không có sinh viên với ID tương ứng
        if (!existsStudent) {
            return "Không có sinh viên có id = " + id;
        }
        // Nếu lớp không tồn tại (ID của lớp là null), trả về thông báo không có lớp với ID tương ứng
        if (clazz.getId() == null) {
        ...
        // Tạo đối tượng Student mới và cập nhật thông tin
